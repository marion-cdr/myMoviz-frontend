...
function Home() {
  ...
  useEffect(() => {
    fetch("https://my-moviz-backend-one-pi.vercel.app/movies")
      .then(res => res.json())
      .then(data => {
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview} poster={path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
